...
def solidworks_csg_rebuild_prompt(requirements: str) -> str:
    ...
    return (
        ...
        "solidworks-mcp.\n"
        "Call solidworks_features_rebuild_csg with one plan (version 1 or 2, "
        "units mm; v1 ops: box, cylinder, cone, cut_cylinder; v2 adds the "
        "first-op-only polygon_prism and swept_arc). Contract: the "
        "first op must be box (v1) or a v2 first-op op at [0,0,0]; solid ops "
        "stack on the axis -- at.z must equal the current "
        "stack top (heights add up); polygon_prism (regular N-sided prism, "
        "stackable) and swept_arc (circular profile along an arc, admits no "
        "following op) require version 2 and must be the only/first op; "
        "cut_cylinder keeps its x/y offset and "
